#!/usr/bin/env node
/**
 * 🚀 Script mejorado para crear un módulo DDD completo (NestJS + Sequelize)
 * Estructura generada:
 * - domain/entities
 * - domain/repositories
 * - domain/services
...
export class ${className}DomainService {
  ...
  async renameEntity(id: string, newName: string): Promise<${className}> {
    const entity = await this.repository.findDomainById(id);
    if (!entity) throw new Error(\`${className} not found\`);
